...
public class ReviewService {
    ...
    @Autowired
    public ReviewService(UserRepository userRepository, NewsRepository newsRepository, ReviewRepository reviewRepository, ReviewMapper reviewMapper) {
    ...
    public ReviewResponse addReview(UpsertReview upsertReview) throws ApiError {
        Optional<UserEntity> optionalUser = userRepository.findById(upsertReview.getUserId());
        if (optionalUser.isEmpty()) {
            throw new ApiError("Пользователь с ID " + upsertReview.getUserId() + " не найден в БД");
        }
